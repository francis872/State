  ...
  return (
    <div className="min-h-screen bg-[#080a12] text-white overflow-x-hidden">
      ...
      <nav className="fixed top-0 inset-x-0 z-50 border-b border-white/5 bg-[#080a12]/90 backdrop-blur-xl">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <Link href="/login" className="text-sm text-slate-400 hover:text-white px-3 py-2 rounded-xl hover:bg-white/5">
              Iniciar sesión
            </Link>
            <Link
              href="/login"
              className="bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-semibold px-4 py-2 rounded-xl shadow-lg shadow-indigo-500/20"
            >
              Comenzar gratis
            </Link>
          </div>
      ...
      <section className="relative pt-28 pb-20 px-4 sm:px-6 text-center overflow-hidden">
        {/* Background blobs */}
        <div className="absolute top-20 left-1/4 w-72 sm:w-96 h-72 sm:h-96 bg-indigo-600/20 rounded-full blur-3xl pointer-events-none" />
        <div className="absolute top-40 right-1/4 w-60 sm:w-80 h-60 sm:h-80 bg-purple-600/15 rounded-full blur-3xl pointer-events-none" />
        ...
        <div className="relative max-w-4xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
            <Link
              href="/login"
              className="w-full sm:w-auto flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-500 text-white font-bold px-8 py-4 rounded-2xl text-lg shadow-lg shadow-indigo-500/30 hover:shadow-indigo-500/50 hover:-translate-y-0.5"
            >
              Comenzar gratis <FiArrowRight size={18} />
            </Link>
            <Link
              href="/dashboard"
              className="w-full sm:w-auto flex items-center justify-center gap-2 bg-white/5 hover:bg-white/10 border border-white/10 text-white font-semibold px-8 py-4 rounded-2xl text-lg hover:-translate-y-0.5"
            >
              Ver demo en vivo
            </Link>
          </div>

          <p className="mt-4 text-sm text-slate-600">Sin tarjeta de crédito · Prueba gratis 14 días</p>
        </div>
      </section>

      {/* ── Stats strip ────────────────────────────────────────────────────── */}
      <section className="border-y border-white/5 py-8 px-4 sm:px-6">
      ...
      <section className="py-20 px-4 sm:px-6">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-12">
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-black mb-4">Todo lo que tu inmobiliaria necesita</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {FEATURES.map(f => (
              <div
                key={f.title}
                className="bg-[#1a1d27] border border-white/5 rounded-2xl p-5 hover:border-indigo-500/30 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-indigo-500/5 group"
              >
                <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${f.color} flex items-center justify-center text-white mb-4 shadow-lg`}>
                  {f.icon}
                </div>
      ...
      {/* ── How it works ───────────────────────────────────────────────────── */}
      <section className="py-20 px-4 sm:px-6 bg-[#0d0f1a]">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-12">
            <h2 className="text-2xl sm:text-3xl font-black mb-4">Listo en 5 minutos</h2>
            <p className="text-slate-400 text-sm sm:text-base">Sin instalaciones. Sin configuraciones complejas.</p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-8">
            {[
              ...
              <div key={item.step} className="text-center relative">
                {i < 2 && <div className="hidden sm:block absolute top-7 left-[60%] right-[-40%] h-px bg-indigo-500/20" />}
      ...
      <section className="py-20 px-4 sm:px-6">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-12">
            <h2 className="text-2xl sm:text-3xl font-black mb-4">Lo que dicen nuestros clientes</h2>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {TESTIMONIALS.map(t => (
              <div key={t.name} className="bg-[#1a1d27] border border-white/5 rounded-2xl p-5 flex flex-col gap-4 hover:border-white/10">
      ...
      {/* ── Pricing ────────────────────────────────────────────────────────── */}
      <section className="py-20 px-4 sm:px-6 bg-[#0d0f1a]" id="precios">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-12">
            <h2 className="text-2xl sm:text-3xl font-black mb-4">Precios simples y transparentes</h2>
            <p className="text-slate-400 text-sm sm:text-base">Sin sorpresas. Cancela cuando quieras.</p>
          </div>
            ...
            {PRICING.map(plan => (
              <div
                ...
                className={`relative bg-[#1a1d27] border rounded-2xl p-6 flex flex-col gap-5
                  ${plan.popular ? 'border-indigo-500 shadow-xl shadow-indigo-500/15 sm:-mt-3 sm:pb-9' : 'border-white/5'}`}
              >
                {plan.popular && (
      ...
      <section className="py-20 px-4 sm:px-6">
        <div className="max-w-3xl mx-auto text-center">
          <div className="bg-gradient-to-br from-indigo-600/20 to-purple-600/20 border border-indigo-500/20 rounded-3xl p-8 sm:p-12">
            <h2 className="text-2xl sm:text-3xl font-black mb-4">¿Listo para cerrar más negocios?</h2>
            ...
            <Link
              href="/login"
              className="inline-flex items-center gap-2 bg-indigo-600 hover:bg-indigo-500 text-white font-bold px-8 py-4 rounded-2xl text-base sm:text-lg shadow-lg shadow-indigo-500/30 hover:-translate-y-0.5"
            >
      ...
      <footer className="border-t border-white/5 py-8 px-4 sm:px-6">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4 text-center md:text-left">
          ...
          <div className="flex gap-6 text-sm text-slate-500">
            ...
            <Link href="mailto:hola@stateos.co" className="hover:text-white">Contacto</Link>
